...
typedef struct tag_TJObj{
  tjhandle              pTJHnd ;
  unsigned char*        pEncBuf ;
  int                   encBufSize ;
  unsigned char*        pDecBuf ;
  int                   decBufSize ;
}TJObj, *PTJObj;
...
int main(int argc, char** argv){
  ...
  TJObj*       tjObjs        = new TJObj[kThdCnt];
  memset(tjObjs, 0, kThdCnt * sizeof(TJObj));
  for(int i = 0 ; i < kThdCnt ; i ++){
    tjObjs[i].pTJHnd = tjInitDecompress() ;

    if (NULL == tjObjs[i].pTJHnd){
  ...
  for(std::size_t t = 0 ; t < kThdCnt ; t ++){
    for(std::size_t i = 0 ; i < jpeg_files_per_thd[t].size() ; i ++){
      ...
      if(NULL == jpegFile){
        ...
        if(NULL != tjObjs[objIdx].pEncBuf){
          if(tjObjs[objIdx].encBufSize < iFileSize){
            tjFree(tjObjs[objIdx].pEncBuf);
            tjObjs[objIdx].pEncBuf     = (unsigned char *)tjAlloc(iFileSize);
            tjObjs[objIdx].encBufSize  = iFileSize ;
          }
        }else{
          tjObjs[objIdx].pEncBuf     = (unsigned char *)tjAlloc(iFileSize);
          tjObjs[objIdx].encBufSize  = iFileSize ;
        }
        fread(tjObjs[objIdx].pEncBuf, iFileSize, 1, jpegFile);
        fflush(jpegFile);
        fclose(jpegFile);
        ...
        int height   = 0;
        int channels = 1;
        int ret = tjDecompressHeader3(tjObjs[objIdx].pTJHnd, tjObjs[objIdx].pEncBuf, iFileSize, &width, &height, &subsamp, &cs);
        if (cs == TJCS_GRAY) channels = 1;
        else channels = 3;
      
        if(0 == ret){
          int             ps     = tjPixelSize[cs];
          if(NULL == tjObjs[objIdx].pDecBuf){
            tjObjs[objIdx].pDecBuf    = (unsigned char *)tjAlloc(width * height * ps);
            tjObjs[objIdx].decBufSize = width * height * ps;
          }else{
            if(tjObjs[objIdx].decBufSize < (width * height * ps)){
              tjFree(tjObjs[objIdx].pDecBuf);
              tjObjs[objIdx].pDecBuf    = (unsigned char *)tjAlloc(width * height * ps);
              tjObjs[objIdx].decBufSize = width * height * ps;
            }
          }
          unsigned char*  dstBuf = (unsigned char *)tjAlloc(width * height * ps);
          ret = tjDecompress2(tjObjs[objIdx].pTJHnd, tjObjs[objIdx].pEncBuf, iFileSize, dstBuf, width, 0, height, cs, TJFLAG_FASTDCT);
        }
        if(0 != ret){
          printf("decode error: %d, %s\n", ret, cur_file_name.c_str());
        }
  ...
  float  fps      = ((double)file_cnt) / (((double)duration) / 1000000.0);
  std::cout << "total_file_size: " << total_file_size << ", run cost(us): " << duration << ", fps(frames/second): " << fps << std::endl;

  for(int i = 0 ; i < kThdCnt ; i ++){
    if (0 != tjObjs[i].pTJHnd)
      tjDestroy(tjObjs[i].pTJHnd);

    // Free the YUV frame
    if (0 != tjObjs[i].pEncBuf)
      tjFree(tjObjs[i].pEncBuf);

    if (0 != tjObjs[i].pDecBuf)
      tjFree(tjObjs[i].pDecBuf);
  }
  delete[] tjObjs;
